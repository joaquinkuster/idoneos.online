 ...
 *   CU-03 — Registrar curso               → GET/POST /docente/curso/nuevo + /docente/curso/guardar
 *
 ...
 *   CU-19 — Buscar unidad                → GET /docente/programas/{programaId}/unidades y GET /docente/curso/{cursoId}/gestionar
 *   CU-20 — Agregar unidad               → POST /docente/programas/{programaId}/unidades/agregar
 *   CU-21 — Modificar unidad             → POST /docente/unidades/{id}/modificar
 *   CU-22 — Quitar unidad                → POST /docente/programas/{programaId}/unidades/{unidadId}/quitar
 *   CU-23 — Buscar cronograma            → GET /docente/programas/{programaId}/cronograma
 *   CU-24 — Modificar cronograma         → POST /docente/programas/{programaId}/cronograma/modificar
 ...
 *   CU-48 — Buscar progreso (docente/admin) → GET /docente/curso/{cursoId}/participantes
 */
@Controller
...
public class DocenteController {
    ...
    public String modificarTerminoGlosario(@PathVariable Integer id,
        ...
        if (tgOpt.isEmpty()) return "redirect:/docente";
    ...
    public String listarProgramas(@PathVariable Integer cursoId, Model model, Authentication auth, RedirectAttributes ra) {
                ...
                programaRepository.findById(programaAnteriorId).ifPresent(ant -> {
                    if (ant.getCronogramas() != null) {
                        for (Cronograma cr : ant.getCronogramas()) {
                            Cronograma clon = new Cronograma(cr.getNumeroOrden(), cr.getSemanasDuracion(), guardado, cr.getUnidad());
    ...
    public String modificarPrograma(@PathVariable Integer id,
        ...
        if (pOpt.isEmpty()) return "redirect:/docente";
    ...
    /**
     * TRAZABILIDAD: CU-19 — Buscar unidad (gestionar contenido de unidades del programa).
     * Actor: Docente / Administrador.
     */
    @GetMapping("/programas/{programaId}/unidades")
    public String listarUnidadesPrograma(@PathVariable Integer programaId, Model model, Authentication auth, RedirectAttributes ra) {
    ...
    /**
     * TRAZABILIDAD: CU-20 — Agregar unidad (Caso A: Nueva unidad / Caso B: Reutilizar existente).
     * Actor: Docente.
     */
    ...
    public String agregarUnidad(@PathVariable Integer programaId,
        ...
        Cronograma nuevoCronograma = new Cronograma(siguienteOrden, semanasDuracion > 0 ? semanasDuracion : 1, programa, unidad);
    ...
    public String quitarUnidad(@PathVariable Integer programaId,
        ...
        if (pOpt.isEmpty() || uOpt.isEmpty()) return "redirect:/docente";
    ...
    public String verCronograma(@PathVariable Integer programaId, Model model, Authentication auth, RedirectAttributes ra) {
        Optional<Programa> pOpt = programaRepository.findById(programaId);
        if (pOpt.isEmpty()) return "redirect:/docente";
    ...
    /**
     * TRAZABILIDAD: CU-24 — Modificar cronograma (orden y duración en semanas de cada unidad).
     * Actor: Docente.
     */
    ...
    public String verParticipantes(@PathVariable Integer cursoId,
        ...
        if (curso.getDocente() != null) docentesEquipo.add(curso.getDocente());
    ...
    /**
     * TRAZABILIDAD: CU-28 — Subir material (grabación, bibliografía, presentación, etc.).
     * Actor: Docente.
     */
    ...
    public String listarMaterialesUnidad(@PathVariable Integer unidadId, Model model, Authentication auth, RedirectAttributes ra) {
        Optional<Unidad> uOpt = unidadService.buscarPorId(unidadId);
        if (uOpt.isEmpty()) return "redirect:/docente";
