 ...
 * invocar Mercado Pago API → registrar pago → activar inscripción → emitir
 * comprobante.
 * Incluye la lógica de descuentos y cupones (CU-49 — Aplicar descuento).
 *
 * NOTAS DE COBERTURA:
 * CU-47 paso 3: la vista de checkout muestra el monto original y el monto con
 * descuento (si aplica).
 * CU-47 paso 5: el procesamiento del pago invoca
 * PagoService.procesarPagoTarjeta() que
 * simula la integración con la API de Mercado Pago.
 * CU-47 paso 6: si el pago es aprobado, se activa la inscripción y se emite un
 * comprobante.
 ...
 * CU-45 — Dar de baja inscripción: implementado en CursoController (POST
 * /cursos/inscripciones/{id}/baja).
 *
 * Aplica reglas de negocio: RN-02 (Pago previo requerido para cursos
 * arancelados).
 */
@Controller
@RequestMapping("/pago")
public class PagoController {
     ...
     * TRAZABILIDAD: CU-49 — Aplicar descuento (muestra el monto con descuento
     * calculado).
     * Actor: Alumno.
     * Precondición: sesión con rol Alumno. El alumno tiene una inscripción en
     * estado "Pendiente de Pago"
     * o accede al checkout directamente desde el catálogo.
     * Flujo paso 2-3: muestra el resumen del curso con monto original y monto final
     * con descuento.
     * Flujo paso 4: presenta el formulario de datos de tarjeta para pago con
     * Mercado Pago.
     * NOTA PARCIAL: CU-47 paso 3 también verifica cupo y ventana de inscripción. No
     * verificado aquí.
     */
    ...
    /**
     * TRAZABILIDAD: CU-47 — Realizar pago (procesamiento del pago y activación de
     * inscripción).
     * Actor: Alumno.
     * Precondición: sesión con rol Alumno. Datos de tarjeta cargados correctamente.
     * Flujo paso 5: invoca PagoService.procesarPagoTarjeta() que simula la
     * integración con Mercado Pago.
     * Flujo paso 6: si el pago es aprobado → inscripción activada + comprobante
     * emitido.
     * Postcondición: Pago registrado con estado "Acreditado". Inscripción activa.
     * Comprobante generado.
     * EX-CU47-01: pago rechazado por la pasarela → no implementado el manejo
     * explícito de rechazo.
     ...
     * Aplica regla RN-02: los cursos arancelados requieren pago previo para activar
     * la inscripción.
     */
    ...
    public String procesarPago(@PathVariable Integer cursoId,
        ...
        // CU-44 paso 5: registra la inscripción (o recupera la existente si ya estaba
        // pendiente).
        Inscripcion inscripcion = inscripcionService.inscribirAlumno(usuario, curso);

        // CU-47 paso 5: calcula monto con descuento y procesa el pago con datos de
        // tarjeta.
     ...
     * Flujo paso 4-5: muestra el estado del pago y presenta el comprobante de la
     * operación.
     * NOTA PARCIAL: CU-46 especifica la descarga del comprobante en formato PDF.
     * La generación del PDF se hace en PagoService; la vista aquí solo muestra el
     * resumen.
     */
