...
/**
 * TRAZABILIDAD — Controller para la autenticación, auto-registro y recuperación de contraseña.
 *
 ...
 *               No implementado; la cuenta queda activa de inmediato. IMPLEMENTADO PARCIALMENTE.
 *             NOTA PARCIAL: CU-81 paso 2 solicita DNI. El modelo no tiene campo DNI en Usuario.
 *               IMPLEMENTADO PARCIALMENTE.
 *
 *   CU-90 — Iniciar sesión → GET /login (vista; POST gestionado por Spring Security)
 *             Actor: Alumno, Docente, Administrador.
 *             EX-CU90-01 (paso 4): credenciales inválidas.
 *             NOTA PARCIAL: CU-90 paso 5 valida límite de sesiones concurrentes. No implementado.
 *             NOTA PARCIAL: CU-90 paso 6 registra sesión con IP y dispositivo (tabla SesionUsuario).
 *               No implementado. IMPLEMENTADO PARCIALMENTE.
 *
 *   CU-91 — Cerrar sesión → GET /login?logout (gestionado por Spring Security)
 *             NOTA PARCIAL: CU-91 paso 2 registra la fecha de fin de sesión. No implementado.
 *
...
public class LoginController {
     ...
     * EX-CU90-01 (paso 4): Spring Security redirige a /login?error cuando las credenciales son inválidas.
     * CU-91 paso 3: Spring Security cierra la sesión y redirige a /login?logout.
     * NOTA PARCIAL: CU-90 paso 6 debería registrar la sesión con IP y dispositivo. No implementado.
     */
    @GetMapping("/login")
     ...
     * Flujo paso 2: muestra el formulario de registro con campos nombre, apellido, correo, contraseña.
     * NOTA PARCIAL: CU-81 paso 2 solicita también DNI. El modelo no tiene campo DNI en Usuario.
     */
    @GetMapping("/registro")
     ...
     * Flujo paso 5: registra la cuenta con rol Alumno (sin validación de correo en esta versión).
     * Flujo paso 6: en prod. envía enlace de validación. En demo: la cuenta queda activa directamente.
     * Postcondición: cuenta registrada con rol Alumno.
     * EX-CU81-01 (paso 4): correo ya registrado → redirect a /registro con mensaje.
     * NOTA PARCIAL: CU-81 pasos 6-8 (validación por enlace de email) no están implementados.
     */
    ...
    public String registrarUsuario(@RequestParam String nombre,
        ...
        try {
            ...
            if (usuarioService.buscarPorCorreo(correo).isPresent()) {
                throw new IllegalArgumentException("EX-CU81-01: Ya existe una cuenta con el correo " + correo + ". Iniciá sesión o recuperá tu contraseña.");
            }
     ...
     * Flujo paso 5: genera token UUID con vigencia de 2 horas y lo almacena en el usuario.
     * Postcondición: en producción → correo enviado con enlace. En demo → token visible.
     * EX-CU92-01 (paso 4): correo no registrado → mensaje genérico (no revela existencia de la cuenta).
    ...
    /**
     * TRAZABILIDAD: CU-92 — Recuperar contraseña (validar token y mostrar formulario).
     * Actor: usuario no autenticado con token válido recibido por email.
     * Flujo paso 7: valida que el token exista y no haya expirado (vigencia 2 horas).
     * EX-CU92-02 (paso 7): token inválido o expirado → redirect a /login con mensaje.
     */
    ...
    public String verResetearContrasena(@RequestParam String token, Model model, RedirectAttributes ra) {
        ...
        if (usuarioOpt.isEmpty() || usuarioOpt.get().getExpiracionToken() == null
            ...
            ra.addFlashAttribute("mensaje", "EX-CU92-02: El enlace de recuperación es inválido o ha expirado. Solicitá uno nuevo.");
            return "redirect:/login";
        }
     ...
     * Flujo paso 8: revalida el token, codifica la nueva contraseña con BCrypt, limpia el token.
