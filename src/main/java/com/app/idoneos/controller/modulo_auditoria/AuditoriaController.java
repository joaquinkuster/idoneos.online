 ...
 * Flujo paso 2: muestra el historial completo de acciones registradas en el
 * sistema.
 * Flujo paso 3: permite filtrar por entidad afectada (ej: "Curso", "Usuario",
 * "Pago").
 * Flujo paso 4-5: recupera y lista los registros ordenados cronológicamente
 * descendente.
 *
 * NOTAS DE COBERTURA:
 * CU-95 paso 3: el filtro por entidad afectada se hace con LIKE (contiene)
 * sobre el campo
 * entidadAfectada. El filtro es case-insensitive.
 * CU-95 paso 4-5: los registros se ordenan por fechaHora de forma descendente.
 * CU-95 paso 3 (filtros adicionales): CU-95 especifica también filtro por
 * acción (tipo de acción),
 * rango de fechas y usuario responsable. No implementados. IMPLEMENTADO
 * PARCIALMENTE.
 * CU-95: la generación de registros de auditoría se realiza mediante
 * AuditoriaInterceptor
 * (o AuditoriaAspect) que persiste automáticamente cada acción relevante.
 *
 * El acceso al endpoint /admin/auditoria está restringido a Administrador por
 * SecurityConfig.
 */
@Controller
...
public class AuditoriaController {
     ...
     * Actor: Administrador.
     * Precondición: sesión con rol Administrador. Existen registros en la tabla
     * Auditoria.
     * Flujo paso 2: muestra el formulario de filtro y el historial de eventos de
     * auditoría.
     * Flujo paso 3: si se proporciona el parámetro "entidad", filtra por entidad
     * afectada (LIKE, case-insensitive).
     ...
     * NOTA PARCIAL: CU-95 especifica filtros adicionales (acción, rango de fechas,
     * usuario responsable).
     * Solo está implementado el filtro por entidad afectada. IMPLEMENTADO
     * PARCIALMENTE.
     */
    @GetMapping
    public String verAuditoria(@RequestParam(required = false) String entidad,
        ...
        if (entidad != null && !entidad.isBlank()) {
            ...
            registros = auditoriaRepository
                    .findByEntidadAfectadaContainingIgnoreCaseOrderByFechaHoraDesc(entidad.trim());
        } else {
