 ...
 * CU-97 — Generar informe de ingresos de un curso → GET
 ...
 * El endpoint /admin/estadisticas retorna JSON para consumo asíncrono desde el
 * frontend.
 *
 * NOTAS DE COBERTURA:
 * CU-96 paso 5: el reporte generado se registra en la tabla Reporte con fecha y
 * usuario responsable.
 * CU-97 paso 5: ídem CU-96.
 * CU-98 paso 4: los indicadores se calculan en tiempo real vía
 * ReportesService.obtenerEstadisticasDashboard().
 * CU-96/CU-97 EX-01: si el curso no existe o no hay datos →
...
public class ReportesController {
    ...
    @Autowired
    private ReporteAlumnosPdfGenerator pdfAlumnos;
    @Autowired
    private ReporteIngresosPdfGenerator pdfIngresos;
    @Autowired
     ...
     * Flujo paso 2-4: recupera los KPIs del sistema (cursos, alumnos, ingresos,
     * inscripciones)
     * y el historial de reportes generados.
     * Flujo paso 5: presenta también los formularios para generar informes (CU-96 y
     * CU-97).
     * Postcondición: panel ejecutivo visible con estadísticas en tiempo real e
     * historial de reportes.
     * NOTA: los KPIs se calculan en ReportesService (no aquí) para mantener la
     ...
     * Actor: Administrador.
     * Precondición: sesión con rol Administrador. El curso existe. Rango de fechas
     * válido.
     * Flujo paso 2-3: el administrador selecciona curso y rango de fechas desde el
     * panel.
     * Flujo paso 4: el sistema genera el PDF con las 3 vistas (listado, aprobados,
     * notas).
     * Flujo paso 5: registra el reporte en el historial de reportes (tabla
     * Reporte).
     * Postcondición: PDF generado y descargado. Reporte registrado en el historial.
     * EX-CU96-01: curso no encontrado o sin datos en el período → HTTP 400 Bad
     * Request.
     ...
     * Postcondición: PDF generado y descargado. Reporte registrado en el historial.
     * EX-CU97-01: curso no encontrado o sin pagos en el período → HTTP 400 Bad
     * Request.
    ...
    /**
     * TRAZABILIDAD: CU-98 — Consultar estadísticas (endpoint JSON para consumo
     * asíncrono).
     * Actor: Administrador.
     * Precondición: sesión con rol Administrador.
     * Flujo paso 4: retorna los KPIs del sistema en formato JSON para consumo desde
     * el frontend.
    ...
    public EstadisticasDashboardDTO obtenerEstadisticas(Authentication auth) {
        // CU-98 paso 4: KPIs en tiempo real para consumo asíncrono (fetch/AJAX desde la
        // vista).
