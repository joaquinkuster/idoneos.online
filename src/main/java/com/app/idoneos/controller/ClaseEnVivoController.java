 ...
 *   CU-66 — Programar clase en vivo    → POST /clase-vivo/programar (con cohorte en dictado y duración estimada)
 *   CU-67 — Modificar clase en vivo    → POST /clase-vivo/{claseId}/modificar (título, fecha/hora, duración)
 *   CU-68 — Cancelar clase en vivo     → POST /clase-vivo/{claseId}/cancelar (cambia a estado Cancelada o notifica)
 *   CU-69 — Dar de baja clase en vivo  → POST /clase-vivo/{claseId}/baja (baja lógica para Docente/Admin)
 *   CU-70 — Iniciar clase en vivo      → POST /clase-vivo/{claseId}/iniciar (genera datos de conexión RTMP)
 *   CU-71 — Finalizar clase en vivo    → POST /clase-vivo/{claseId}/finalizar (cambia a Finalizada y genera material)
 *   CU-72 — Ingresar a clase en vivo   → GET /clase-vivo/{claseId}/ver (verificación de alumno y acceso a sala)
 */
@Controller
...
public class ClaseEnVivoController {
    ...
    public String programar(@RequestParam(required = false) Integer cohorteId,
        ...
        ra.addFlashAttribute("mensaje", "Clase '" + titulo + "' programada para " + dt.format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")));
        return "redirect:/clase-vivo/docente";
    }
