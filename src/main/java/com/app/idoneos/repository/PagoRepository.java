...
public interface PagoRepository extends JpaRepository<Pago, Integer> {
     ...
     */
    @Query("SELECT p FROM Pago p WHERE p.inscripcion.dictado.programa.curso = :curso " +
     ...
     */
    @Query("SELECT p.inscripcion.dictado.programa.curso.id, SUM(p.monto) FROM Pago p " +
           "WHERE p.estadoPago.nombre = 'Acreditado' AND p.fecha >= :desde AND p.fecha < :hasta " +
           "GROUP BY p.inscripcion.dictado.programa.curso.id")
     ...
     */
    @Query("SELECT p.inscripcion.dictado.programa.curso.categoria.nombre, SUM(p.monto) FROM Pago p " +
           "WHERE p.estadoPago.nombre = 'Acreditado' AND p.fecha >= :desde AND p.fecha < :hasta " +
           "GROUP BY p.inscripcion.dictado.programa.curso.categoria.nombre")
