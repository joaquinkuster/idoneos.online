...
// 1. Leer el HTML maestro
const htmlPath = path.join(__dirname, '..', 'docs', 'diseño', 'Pantallas_CU_Reales.html');
const fullHtml = fs.readFileSync(htmlPath, 'utf8');

// 2. Extraer el bloque <style> maestro
const styleMatch = fullHtml.match(/<style>([\s\S]*?)<\/style>/);
const masterCss = styleMatch ? styleMatch[1] : '';

// 3. Mapeo de Módulos a carpetas de templates/pages
const moduleFolderMap = {
  'MOD-F-01': 'cursos',
  'MOD-F-02': 'academico',
  'MOD-F-03': 'inscripciones',
  'MOD-F-04': 'pagos',
...
// Directorio global /pages/cu/
const allPagesDir = path.join(baseDir, 'cu');
if (!fs.existsSync(allPagesDir)) fs.mkdirSync(allPagesDir, { recursive: true });
...
for (let i = 1; i < figureBlocks.length; i++) {
  ...
  else if (['CU-15','CU-16','CU-17','CU-18','CU-19','CU-20','CU-21','CU-22','CU-23','CU-24','CU-25','CU-26','CU-26b','CU-27','CU-28','CU-29','CU-30','CU-31','CU-32','CU-33','CU-34','CU-35','CU-36','CU-37','CU-38','CU-39','CU-40','CU-41','CU-42'].includes(cuId)) folder = 'academico';
...
` + masterCss + `
        ...
        .screen-frame {
            box-shadow: 0 12px 36px rgba(8, 20, 38, 0.1);
        }
    </style>
</head>
<body>

    <div class="cu-page-container">
...
`;
  ...
  const fileName = `${cuId.toLowerCase().replace(/[^a-z0-9_-]/g, '_')}.html`;
  fs.writeFileSync(path.join(baseDir, folder, fileName), pageTemplate, 'utf8');
  fs.writeFileSync(path.join(allPagesDir, fileName), pageTemplate, 'utf8');

  count++;
}

console.log(`¡Éxito total! Se generaron exactamente ${count} vistas individuales en src/main/resources/templates/pages/`);
