...
// 2. Replace isDelete implementation
...
// 3. Inject specializedFormGenerators lookup in generateScreenContent
const formCheckSnippet = `
  if (specializedFormGenerators && specializedFormGenerators[id]) {
    return specializedFormGenerators[id](cu, badges);
  }
`;

if (!code.includes("specializedFormGenerators[id]")) {
  const genContentFuncStart = code.indexOf('function generateScreenContent(cu) {');
  if (genContentFuncStart !== -1) {
    const insertPos = code.indexOf('{', genContentFuncStart) + 1;
    code = code.substring(0, insertPos) + formCheckSnippet + code.substring(insertPos);
  }
}

// 4. Update isSearch button labels to remove + prefixes and ensure Clean labels
code = code.replace("createBtnLabel = '+ Nueva Categoría';", "createBtnLabel = 'Nueva Categoría';");
code = code.replace("createBtnLabel = '+ Nueva Cohorte';", "createBtnLabel = 'Nueva Cohorte';");
code = code.replace("createBtnLabel = '+ Nuevo Programa';", "createBtnLabel = 'Nuevo Programa';");
code = code.replace("createBtnLabel = '+ Nuevo Descuento';", "createBtnLabel = 'Nuevo Descuento';");
code = code.replace("createBtnLabel = '+ Nuevo Usuario';", "createBtnLabel = 'Nuevo Usuario';");
code = code.replace("createBtnLabel = '+ Nuevo Docente';", "createBtnLabel = 'Nuevo Docente';");

// 5. Update Hero Banner in generate_screens.js for all other searches
const heroSearchButtons = `
                    ...
                    \` : ''}
                    \${cu.id === 'CU-11' || cu.id === 'CU-15' ? \`
                    <div class="d-flex align-items-center gap-2">
                        <a href="#\${cu.id === 'CU-11' ? 'CU-12' : 'CU-16'}" class="wf-btn-gold">
                            <i class="fa-solid fa-plus"></i>
                            <span>\${cu.id === 'CU-11' ? 'Nueva Cohorte' : 'Nuevo Programa'}</span>
                        </a>
                        <span class="pin-badge">A</span>
                    </div>
                    \` : ''}
...
console.log('Successfully updated generate_screens.js with all specialized forms and clean search banners.');
