...
async function runCompleteTestSuite() {
  console.log(`\n======================================================`);
  console.log(`🏆 AUDITORÍA Y CERTIFICACIÓN COMPLETA DE TODOS LOS CASOS DE USO`);
  console.log(`======================================================\n`);
  ...
  console.log(`- Alumno Session: ${alumnoCookie ? 'OK ✅' : 'FAIL ❌'}`);
  console.log(`- Docente Session: ${docenteCookie ? 'OK ✅' : 'FAIL ❌'}`);
  console.log(`- Admin Session: ${adminCookie ? 'OK ✅' : 'FAIL ❌'}\n`);
  ...
  for (const t of tests) {
    ...
    const isWhitelabel = res.body.includes('Whitelabel Error Page') || res.body.includes('TemplateInputException') || res.body.includes('Exception processing template');
    const isOk = res.status === 200 && !isWhitelabel;

    if (isOk) {
      console.log(`✅ [${res.status}] ${t.cu}: ${t.name} -> ${t.url}`);
      passed++;
    } else {
      console.log(`❌ [${res.status}] ${t.cu}: ${t.name} -> ${t.url} (Whitelabel: ${isWhitelabel})`);
      failed++;
    }
  }

  console.log(`\n======================================================`);
  console.log(`📊 REPORTE DE EVALUACIÓN FINAL DE RUTAS Y SERVICIOS:`);
  console.log(`- Total de Pruebas: ${tests.length}`);
  console.log(`- Pruebas Exitosas (HTTP 200 OK): ${passed}`);
  console.log(`- Pruebas Fallidas: ${failed}`);
  console.log(`- Tasa de Éxito Funcional: ${((passed / tests.length) * 100).toFixed(1)}%`);
  console.log(`======================================================\n`);
}
