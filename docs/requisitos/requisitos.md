...
En esta sección se detallan los 93 casos de uso extendidos del Sistema Idóneos Online, organizados según los diez módulos definidos en la Nota de Presentación (seis funcionales y cuatro no funcionales). Cada caso de uso se referencia mediante un código correlativo (CU-01 a CU-93).
...
*   **Excepciones:**
    ...
    | 5 | Si el precio ingresado es menor a cero, el sistema informa el error y vuelve al paso 3. |
    | 6 | Si se intenta publicar un curso sin al menos un programa con al menos 10 unidades con material publicado, el sistema informa el error y vuelve al paso 3. |
...
*   **Módulo:** MOD-F-01: Módulo del Catálogo de Cursos
*   **Actor(es):** Administrador
*   **Descripción:** Permite al Administrador dar de baja un curso. Si el curso posee dictados con alumnos con inscripción vigente, el sistema informa la dependencia y no permite la baja.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Administrador.
    *   El curso existe en el sistema y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el curso a dar de baja (ver CU-01: Buscar curso).
    2.  El sistema verifica que no existan inscripciones vigentes en ningún dictado del curso.
...
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | 2 | Si el curso posee dictados con alumnos con inscripción vigente, el sistema informa la dependencia y no permite la baja. |
...
*   **Flujo de eventos:**
    ...
    5.  Fin del caso de uso.
*   **Salida:** Se recupera el listado de programas del curso seleccionado, con su nombre, descripción, meses de acceso e indicación de cuál es el vigente.
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | - | - |
...
*   **Estabilidad:** Alta
*   **Comentarios:** Solo el programa vigente admite nuevos dictados (ver CU-16: Registrar dictado); los programas anteriores se conservan para no afectar a los alumnos ya inscritos en dictados de una edición previa.
...
*   **Módulo:** MOD-F-01: Módulo del Catálogo de Cursos
*   **Actor(es):** Administrador
*   **Descripción:** Permite al Administrador registrar un nuevo programa para un curso, definiendo una nueva versión de su plan de estudios.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Administrador.
...
*   **Flujo de eventos:**
    ...
    3.  El actor ingresa los datos solicitados.
    4.  El sistema valida que el nombre y los meses de acceso hayan sido completados.
    5.  El sistema valida que los meses de acceso sean un valor entero mayor a cero.
    6.  El sistema registra el programa, que pasa a ser el vigente del curso al ser el de fecha de creación más reciente, dejando de considerarse vigente al programa anterior, si existía.
    7.  El sistema informa el éxito del registro.
    8.  Fin del caso de uso.
*   **Postcondición(es):**
...
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | 4 | Si el nombre o los meses de acceso no fueron completados, el sistema informa el error y vuelve al paso 3. |
    | 5 | Si los meses de acceso ingresados no son un número entero mayor a cero, el sistema informa el error y vuelve al paso 3. |
*   **Frecuencia:** Baja — ocurre al definir el contenido inicial de un curso o al actualizarlo en una nueva edición.
*   **Estabilidad:** Alta
*   **Comentarios:** El contenido del nuevo programa (unidades) se carga por separado mediante CU-22: Registrar unidad; no se copia automáticamente del programa anterior.
...
*   **Módulo:** MOD-F-01: Módulo del Catálogo de Cursos
*   **Actor(es):** Administrador
*   **Descripción:** Permite al Administrador modificar el nombre, la descripción o los meses de acceso de un programa.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Administrador.
...
*   **Flujo de eventos:**
    ...
    3.  El actor modifica el nombre, la descripción o los meses de acceso.
    4.  El sistema valida que el nombre y los meses de acceso no queden vacíos.
    5.  El sistema valida que los meses de acceso sean un valor entero mayor a cero.
...
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | 4 | Si el nombre o los meses de acceso quedan vacíos, el sistema informa el error y vuelve al paso 3. |
    | 5 | Si los meses de acceso ingresados no son un número entero mayor a cero, el sistema informa el error y vuelve al paso 3. |
...
*   **Módulo:** MOD-F-01: Módulo del Catálogo de Cursos
*   **Actor(es):** Administrador
*   **Descripción:** Permite al Administrador dar de baja un programa. Si el programa posee dictados con alumnos con inscripción vigente, el sistema informa la dependencia y no permite la baja.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Administrador.
    *   El programa existe y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el programa a dar de baja (ver CU-10: Buscar programa).
    2.  El sistema verifica que no existan inscripciones vigentes en ningún dictado del programa.
    3.  El actor confirma la baja.
    4.  El sistema marca el programa como dado de baja.
    5.  El sistema informa el éxito de la operación.
    6.  Fin del caso de uso.
*   **Postcondición(es):** El programa queda en estado de baja.
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | 2 | Si el programa posee dictados con alumnos con inscripción vigente, el sistema informa la dependencia y no permite la baja. |
    | 3 | Si el actor no confirma la baja, el sistema cancela la operación y finaliza el caso de uso. |
*   **Frecuencia:** Baja — ocurre cuando una versión del plan de estudios queda obsoleta.
*   **Estabilidad:** Alta
*   **Comentarios:** –

### CU-14 Cambiar programa
*   **Objetivo(s) asociado(s):** OBJ-01: Gestionar y explorar el catálogo de cursos.
*   **Requisito(s) de información asociado(s):** RI-01: Información sobre el catálogo de cursos.
*   **Módulo:** MOD-F-01: Módulo del Catálogo de Cursos
*   **Actor(es):** Docente, Administrador
...
*   **Flujo de eventos:**
    ...
    3.  El sistema recupera y lista las unidades del programa seleccionado (ver CU-19: Buscar unidad).
...
*   **Excepciones:**
    ...
    | 5 | Si el docente indicado como titular, o el supervisor si corresponde, no se encuentran habilitados, el sistema informa el error y vuelve al paso 3. |
    | 6 | Si la fecha de fin no es posterior a la fecha de inicio, el sistema informa el error y vuelve al paso 3. |
    | 7 | Si el cupo máximo ingresado no es un número entero mayor a cero, el sistema informa el error y vuelve al paso 3. |
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el dictado a modificar (ver CU-15: Buscar dictado).
...
*   **Excepciones:**
    ...
    | :--- | :--- |
    | 4 | Si algún campo obligatorio queda vacío, el sistema informa el error y vuelve al paso 3. |
    | 5 | Si el docente indicado como titular, o el supervisor, no se encuentran habilitados, el sistema informa el error y vuelve al paso 3. |
    | 6 | Si la fecha de fin no es posterior a la fecha de inicio, el sistema informa el error y vuelve al paso 3. |
    | 7 | Si el cupo máximo ingresado no es un número entero mayor a cero, el sistema informa el error y vuelve al paso 3. |
...
### CU-18 Eliminar dictado
*   **Objetivo(s) asociado(s):** OBJ-01: Gestionar y explorar el catálogo de cursos.
*   **Requisito(s) de información asociado(s):** RI-01: Información sobre el catálogo de cursos.
*   **Módulo:** MOD-F-01: Módulo del Catálogo de Cursos
*   **Actor(es):** Administrador
*   **Descripción:** Permite al Administrador dar de baja (cancelar) un dictado. Si el dictado posee alumnos con inscripción vigente, el sistema informa la dependencia y no permite la baja.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Administrador.
    *   El dictado existe y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el dictado a dar de baja (ver CU-15: Buscar dictado).
    2.  El sistema verifica que no existan alumnos con inscripción vigente en el dictado.
    3.  El actor confirma la baja.
...
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | 2 | Si el dictado posee alumnos con inscripción vigente, el sistema informa la dependencia y no permite la baja. |
    | 3 | Si el actor no confirma la baja, el sistema cancela la operación y finaliza el caso de uso. |
*   **Frecuencia:** Baja — ocurre cuando una edición programada no llega a dictarse.
*   **Estabilidad:** Alta
*   **Comentarios:** –

## MOD-F-02: Módulo de Estructura y Contenido de Unidades

### CU-19 Buscar unidad
*   **Objetivo(s) asociado(s):** OBJ-02: Gestionar la estructura y el contenido de los cursos.
*   **Requisito(s) de información asociado(s):** RI-02: Información sobre la estructura y el contenido de los cursos.
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Docente, Administrador
*   **Descripción:** Permite al Docente titular/supervisor o al Administrador buscar las unidades de un programa de un curso, con fines de gestión de su estructura y contenido.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Docente o Administrador.
...
*   **Flujo de eventos:**
    ...
    3.  El sistema utiliza el programa vigente del curso, salvo que el actor lo haya cambiado previamente (ver CU-14: Cambiar programa).
...
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Docente, Administrador
*   **Descripción:** Permite al Docente titular/supervisor o al Administrador buscar las unidades de un curso con fines de gestión de contenido, para acceder al material, los términos de glosario, los pools, las autoevaluaciones y las clases de cada una, y registrarlos, modificarlos o eliminarlos.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Docente o Administrador.
    *   El programa existe y posee al menos una unidad registrada.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca las unidades de un curso con fines de gestión de contenido (ver CU-19: Buscar unidad).
    2.  El sistema lista las unidades del programa, con acceso, para cada una, a su material, glosario, foro, pools, autoevaluaciones y clases.
    ...
    5.  Fin del caso de uso.
*   **Salida:** Se recupera el listado de unidades del curso con acceso para gestionar el contenido de cada una. Al seleccionar una unidad, se despliega su material, su glosario, su foro,. sus pools, sus autoevaluaciones y sus clases, con las opciones para gestionarlos.
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | - | - |
...
*   **Estabilidad:** Alta
*   **Comentarios:** Las altas de contenido dentro de este modo solo se permiten si la unidad pertenece al programa vigente del curso (ver CU-26: Subir material y análogos); las modificaciones y bajas no tienen esa restricción.
...
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | 4 | Si la unidad todavía no está habilitada para el alumno, el sistema informa que debe aprobar primero la autoevaluación de la unidad anterior y no despliega su contenido. |
...
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Docente, Administrador
*   **Descripción:** Permite al Docente titular/supervisor o al Administrador registrar una nueva unidad dentro de un programa, definiendo su posición en el avance secuencial.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Docente o Administrador.
    *   El programa existe, se encuentra vigente y no en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el curso para el cual desea registrar una nueva unidad en su programa vigente (ver CU-01: Buscar curso).
    ...
    5.  El sistema valida que el número de orden sea un valor entero mayor a cero y que no esté ya utilizado por otra unidad del mismo programa.
    6.  El sistema registra la unidad.
    7.  El sistema informa el éxito del registro.
    8.  Fin del caso de uso.
*   **Postcondición(es):**
    *   La unidad queda registrada y asociada al programa.
    *   La fecha de creación refleja el momento del alta.
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    ...
    | 5 | Si el número de orden ya está utilizado por otra unidad del programa, el sistema informa el error y vuelve al paso 3. |
*   **Frecuencia:** Media — ocurre al estructurar el contenido de cada curso nuevo.
*   **Estabilidad:** Alta
*   **Comentarios:** El programa debe contar con un mínimo de 10 unidades para que el curso pueda publicarse, según lo relevado con el cliente; esta validación se controla en CU-03: Modificar curso al momento de la publicación.
...
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Docente, Administrador
*   **Descripción:** Permite al Docente titular/supervisor o al Administrador modificar el título, la descripción o el número de orden de una unidad.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Docente o Administrador.
    *   La unidad existe y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la unidad a modificar (ver CU-19: Buscar unidad).
    ...
    6.  El sistema valida que, si se modificó el número de orden, no coincida con el de otra unidad del mismo programa.
...
*   **Excepciones:**
    ...
    | 5 | Si el número de orden ingresado no es un entero mayor a cero, el sistema informa el error y vuelve al paso 3. |
    | 6 | Si el número de orden coincide con el de otra unidad del programa, el sistema informa el error y vuelve al paso 3. |
*   **Frecuencia:** Baja — se usa para reordenar o corregir unidades ya cargadas.
*   **Estabilidad:** Alta
*   **Comentarios:** –

### CU-24 Eliminar unidad
*   **Objetivo(s) asociado(s):** OBJ-02: Gestionar la estructura y el contenido de los cursos.
*   **Requisito(s) de información asociado(s):** RI-02: Información sobre la estructura y el contenido de los cursos.
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Docente, Administrador
*   **Descripción:** Permite al Docente titular/supervisor o al Administrador dar de baja una unidad. Si algún alumno ya registra un intento de autoevaluación aprobado en esa unidad, el sistema informa la dependencia y no permite la baja.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Docente o Administrador.
    *   La unidad existe y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la unidad a dar de baja (ver CU-19: Buscar unidad).
    ...
    5.  El sistema informa el éxito de la operación.
    6.  Fin del caso de uso.
*   **Postcondición(es):** La unidad queda en estado de baja y deja de estar disponible para los alumnos.
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    ...
    | 3 | Si el actor no confirma la baja, el sistema cancela la operación y finaliza el caso de uso. |
*   **Frecuencia:** Baja — se usa cuando el docente decide reorganizar o eliminar contenido de un curso aún no cursado.
*   **Estabilidad:** Alta
*   **Comentarios:** –

### CU-25 Buscar material
*   **Objetivo(s) asociado(s):** OBJ-02: Gestionar la estructura y el contenido de los cursos.
*   **Requisito(s) de información asociado(s):** RI-02: Información sobre la estructura y el contenido de los cursos.
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Docente, Administrador
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor desea subir un nuevo material dentro de una unidad (ver CU-19: Buscar unidad).
...
*   **Estabilidad:** Alta
*   **Comentarios:** El material de tipo Grabación también puede originarse automáticamente desde CU-66: Finalizar clase en vivo o CU-72: Generar clase con Clon IA; el material de tipo Presentación y Resumen también puede generarse desde el Módulo de Generación de Contenido con IA. Este CU cubre específicamente la carga manual por parte del docente.

### CU-27 Modificar material
*   **Objetivo(s) asociado(s):** OBJ-02: Gestionar la estructura y el contenido de los cursos.
*   **Requisito(s) de información asociado(s):** RI-02: Información sobre la estructura y el contenido de los cursos.
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Docente
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el material a modificar (ver CU-25: Buscar material).
...
*   **Precondición(es):**
    ...
    *   El material existe y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el material a dar de baja (ver CU-25: Buscar material).
...
### CU-29 Buscar término de glosario
*   **Objetivo(s) asociado(s):** OBJ-02: Gestionar la estructura y el contenido de los cursos.
*   **Requisito(s) de información asociado(s):** RI-02: Información sobre la estructura y el contenido de los cursos.
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Docente, Administrador
...
*   **Flujo de eventos:**
    ...
    2.  El sistema solicita la unidad sobre la que se desea consultar y, opcionalmente, el término o la definición.
    3.  El actor ingresa los criterios de búsqueda que desea.
...
*   **Precondición(es):**
    ...
    *   La unidad existe, no se encuentra en baja y pertenece al programa vigente del curso.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor desea registrar un término de glosario dentro de una unidad (ver CU-19: Buscar unidad).
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el término de glosario a modificar (ver CU-29: Buscar término de glosario).
...
### CU-32 Eliminar término de glosario
*   **Objetivo(s) asociado(s):** OBJ-02: Gestionar la estructura y el contenido de los cursos.
*   **Requisito(s) de información asociado(s):** RI-02: Información sobre la estructura y el contenido de los cursos.
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Docente, Administrador
...
*   **Precondición(es):**
    ...
    *   El término de glosario existe y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el término de glosario a dar de baja (ver CU-29: Buscar término de glosario).
    2.  El actor confirma la baja.
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor solicita buscar consultas de foro dentro de una unidad.
    2.  El sistema solicita la unidad sobre la que se desea consultar y, opcionalmente, el texto o el rango de fechas.
    3.  El actor ingresa los criterios de búsqueda que desea.
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la consulta de foro propia a modificar (ver CU-33: Buscar consulta de foro).
...
### CU-36 Eliminar consulta de foro
*   **Objetivo(s) asociado(s):** OBJ-02: Gestionar la estructura y el contenido de los cursos.
*   **Requisito(s) de información asociado(s):** RI-02: Información sobre la estructura y el contenido de los cursos.
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Administrador
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la consulta de foro a dar de baja (ver CU-33: Buscar consulta de foro).
    2.  El actor confirma la baja.
...
### CU-37 Buscar respuesta de foro
*   **Objetivo(s) asociado(s):** OBJ-02: Gestionar la estructura y el contenido de los cursos.
*   **Requisito(s) de información asociado(s):** RI-02: Información sobre la estructura y el contenido de los cursos.
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Alumno, Docente, Administrador
*   **Descripción:** Permite consultar las respuestas registradas a una consulta de foro. La vista varía según el rol del actor, con el mismo criterio que CU-33: Buscar consulta de foro.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Alumno, Docente o Administrador.
...
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | 2 | Si el actor es Alumno y no posee inscripción vigente al curso de la unidad, el sistema no muestra las respuestas. |
*   **Frecuencia:** Alta — se consulta junto con CU-33: Buscar consulta de foro.
*   **Estabilidad:** Alta
*   **Comentarios:** –
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor solicita responder una consulta de foro (ver CU-33: Buscar consulta de foro).
...
*   **Precondición(es):**
    ...
    *   No se superó el plazo límite de edición configurado.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la respuesta de foro propia a modificar (ver CU-37: Buscar respuesta de foro).
...
### CU-40 Eliminar respuesta de foro
*   **Objetivo(s) asociado(s):** OBJ-02: Gestionar la estructura y el contenido de los cursos.
*   **Requisito(s) de información asociado(s):** RI-02: Información sobre la estructura y el contenido de los cursos.
*   **Módulo:** MOD-F-02: Módulo de Estructura y Contenido de Unidades
*   **Actor(es):** Administrador
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la respuesta de foro a dar de baja (ver CU-37: Buscar respuesta de foro).
    2.  El actor confirma la baja.
...
## MOD-F-03: Módulo de Inscripción y Pagos

### CU-41 Buscar inscripción
*   **Objetivo(s) asociado(s):** OBJ-03: Gestionar la inscripción y el pago de los alumnos.
*   **Requisito(s) de información asociado(s):** RI-03: Información sobre inscripciones y pagos.
*   **Módulo:** MOD-F-03: Módulo de Inscripción y Pagos
...
*   **Flujo de eventos:**
    ...
    6.  Si el curso tiene costo, el sistema deriva al alumno al pago (CU-45: Realizar pago) y aguarda su resultado.
...
*   **Postcondición(es):**
    ...
    *   El acceso al contenido del curso permanece bloqueado hasta que se confirme el pago mediante CU-45: Realizar pago.
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | 2 | Si la fecha actual no es anterior a la fecha de inicio del dictado, el sistema informa que la inscripción a ese dictado ya no está habilitada y finaliza el caso de uso. |
    | 3 | Si el dictado ya alcanzó su cupo máximo, el sistema lo informa y finaliza el caso de uso. |
    | 4 | Si el alumno ya posee una inscripción vigente a ese dictado, el sistema lo informa y finaliza el caso de uso. |
    | 6 | En caso de que el curso sea gratuito, se omite el paso 6 y el sistema continúa directamente al paso 7. |
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la inscripción a dar de baja (ver CU-41: Buscar inscripción).
...
*   **Requisito(s) de información asociado(s):** RI-03: Información sobre inscripciones y pagos.
*   **Módulo:** MOD-F-03: Módulo de Inscripción y Pagos
*   **Actor(es):** Alumno, indirectamente mediante CU-42: Inscribir curso
...
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | 5 | Si el alumno cancela el pago desde el modal de MODO, el sistema informa que la operación fue cancelada y finaliza el caso de uso. |
    | 7 | Si MODO rechaza el pago, el sistema registra el pago como rechazado y notifica al alumno el motivo, permitiéndole reintentar. |
    | 7 | Si no se recibe la confirmación de MODO dentro del plazo configurado (ver CU-93: Configurar parámetros), el pago queda pendiente y el sistema notifica al alumno que su pago está en proceso. |
...
### CU-46 Buscar progreso
*   **Objetivo(s) asociado(s):** OBJ-03: Gestionar la inscripción y el pago de los alumnos.
*   **Requisito(s) de información asociado(s):** RI-03: Información sobre inscripciones y pagos.
*   **Módulo:** MOD-F-03: Módulo de Inscripción y Pagos
*   **Actor(es):** Alumno, Docente, Administrador
*   **Descripción:** Permite consultar el progreso de un alumno en las unidades de un curso en el que está inscripto. La vista varía según el rol del actor: el Alumno visualiza únicamente el propio; el Docente y Administrador visualiza el de cualquier alumno.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Alumno, Docente o Administrador.
...
*   **Flujo de eventos:**
    ...
    6.  Fin del caso de uso.
*   **Salida:** Se recupera, para cada unidad del curso, si fue completada y, en caso afirmativo, la fecha en que se completó.
*   **Excepciones:**
    | Paso | Acción |
    | :--- | :--- |
    | - | - |
...
*   **Estabilidad:** Alta
*   **Comentarios:** El progreso de una unidad se registra automáticamente al aprobar el intento de autoevaluación correspondiente (ver CU-60: Realizar intento de autoevaluación).

### CU-47 Buscar descuento
*   **Objetivo(s) asociado(s):** OBJ-03: Gestionar la inscripción y el pago de los alumnos.
*   **Requisito(s) de información asociado(s):** RI-03: Información sobre inscripciones y pagos.
*   **Módulo:** MOD-F-03: Módulo de Inscripción y Pagos
*   **Actor(es):** Administrador
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el descuento a modificar (ver CU-47: Buscar descuento).
...
### CU-50 Eliminar descuento
*   **Objetivo(s) asociado(s):** OBJ-03: Gestionar la inscripción y el pago de los alumnos.
*   **Requisito(s) de información asociado(s):** RI-03: Información sobre inscripciones y pagos.
*   **Módulo:** MOD-F-03: Módulo de Inscripción y Pagos
*   **Actor(es):** Administrador
*   **Descripción:** Permite al Administrador dar de baja un descuento antes de su vencimiento natural.
*   **Precondición(es):**
    *   El actor ha iniciado sesión en el sistema con el rol Administrador.
    *   El descuento existe y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el descuento a dar de baja (ver CU-47: Buscar descuento).
    2.  El actor confirma la baja.
...
*   **Estabilidad:** Alta
*   **Comentarios:** La baja no afecta los descuentos ya aplicados a inscripciones previas.

## MOD-F-04: Módulo de Evaluación y Certificación

### CU-51 Buscar pool
*   **Objetivo(s) asociado(s):** OBJ-04: Evaluar y certificar a los alumnos.
*   **Requisito(s) de información asociado(s):** RI-04: Información sobre evaluación y certificación.
*   **Módulo:** MOD-F-04: Módulo de Evaluación y Certificación
*   **Actor(es):** Docente, Administrador
...
*   **Precondición(es):**
    ...
    *   La unidad existe, no se encuentra en baja y pertenece al programa vigente del curso.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente desea crear un nuevo pool dentro de una unidad (ver CU-19: Buscar unidad).
...
*   **Estabilidad:** Alta
*   **Comentarios:** El pool también puede generarse automáticamente a partir del material de la unidad mediante CU-68: Generar banco de preguntas, como método alternativo a la carga manual.

### CU-53 Modificar pool
*   **Objetivo(s) asociado(s):** OBJ-04: Evaluar y certificar a los alumnos.
*   **Requisito(s) de información asociado(s):** RI-04: Información sobre evaluación y certificación.
*   **Módulo:** MOD-F-04: Módulo de Evaluación y Certificación
*   **Actor(es):** Docente
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el pool a modificar (ver CU-51: Buscar pool).
...
*   **Precondición(es):**
    ...
    *   El pool existe y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el pool a dar de baja (ver CU-51: Buscar pool).
...
*   **Precondición(es):**
    ...
    *   La unidad existe, no se encuentra en baja y pertenece al programa vigente del curso.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente desea crear una autoevaluación dentro de una unidad (ver CU-19: Buscar unidad)
...
*   **Excepciones:**
    ...
    | 5 | Si el tiempo límite, o la cantidad de intentos cuando se indicó, no son valores enteros mayores a cero, el sistema informa el error y vuelve al paso 3. |
    ...
    | 7 | Si los pools seleccionados no reúnen al menos 10 preguntas activas en conjunto, el sistema informa el error y vuelve al paso 3. |
...
*   **Estabilidad:** Alta
*   **Comentarios:** Cada intento de un alumno sortea 10 preguntas de los pools asociados a la autoevaluación (ver CU-60: Realizar intento de autoevaluación).

### CU-57 Modificar autoevaluación
*   **Objetivo(s) asociado(s):** OBJ-04: Evaluar y certificar a los alumnos.
*   **Requisito(s) de información asociado(s):** RI-04: Información sobre evaluación y certificación.
*   **Módulo:** MOD-F-04: Módulo de Evaluación y Certificación
*   **Actor(es):** Docente
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la autoevaluación a modificar (ver CU-55: Buscar autoevaluación).
...
*   **Excepciones:**
    ...
    | 5 | Si el tiempo límite, o la cantidad de intentos cuando se indicó, no son valores enteros mayores a cero, el sistema informa el error y vuelve al paso 3. |
    | 6 | Si la fecha de cierre especificada no es posterior a la fecha de apertura, el sistema informa el error y vuelve al paso 3. |
    | 7 | Si los pools asociados no reúnen al menos 10 preguntas activas, el sistema informa el error y vuelve al paso 3. |
...
*   **Precondición(es):**
    ...
    *   La autoevaluación existe y no se encuentra en baja.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la autoevaluación a dar de baja (ver CU-55: Buscar autoevaluación).
...
*   **Estabilidad:** Alta
*   **Comentarios:** La aprobación exige responder correctamente la totalidad de las 10 preguntas del intento, según lo confirmado por el cliente; no existe una nota de corte parcial. Si la autoevaluación tiene un límite de intentos y el alumno los agota sin aprobar, queda bloqueado para continuar el curso hasta que el Docente amplíe sus intentos mediante CU-57: Modificar autoevaluación.

## MOD-F-05: Módulo de Clases en Vivo

### CU-61 Buscar clase en vivo
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente desea programar una clase en vivo dentro de una unidad (ver CU-19: Buscar unidad).
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la clase en vivo programada a modificar (ver CU-61: Buscar clase en vivo).
...
*   **Precondición(es):**
    ...
    *   La clase en vivo existe, fue registrada por el actor y se encuentra en estado Programada.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la clase en vivo programada a cancelar (ver CU-61: Buscar clase en vivo).
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente busca y selecciona la clase en vivo programada (ver CU-61: Buscar clase en vivo) y, en el horario programado, solicita iniciarla.
    ...
    5.  El sistema recibe la señal transmitida y la redistribuye en simultáneo a los alumnos inscriptos que ingresan a la clase (ver CU-67: Ingresar a clase en vivo), mientras graba automáticamente la transmisión mediante el protocolo RTMP desde OBS.
    6.  Fin del caso de uso.
*   **Postcondición(es):**
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente busca y selecciona la clase en vivo que se encuentra en curso (ver CU-61: Buscar clase en vivo) y solicita finalizar la transmisión.
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente desea generar un banco de preguntas dentro de una unidad (ver CU-19: Buscar unidad).
...
*   **Frecuencia:** Media — se usa como alternativa a la carga manual de preguntas (CU-52: Crear pool).
*   **Estabilidad:** Media — depende de la disponibilidad y el desempeño del modelo de inteligencia artificial local.
*   **Comentarios:** El pool queda registrado sin publicar como pool de referencia hasta que el docente lo revise; su uso efectivo en una autoevaluación se define mediante CU-56: Crear autoevaluación.
...
*   **Precondición(es):**
    ...
    *   La unidad pertenece al programa vigente del curso.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente desea generar un resumen dentro de una unidad (ver CU-19: Buscar unidad).
...
*   **Estabilidad:** Media — depende de la disponibilidad y el desempeño del modelo de inteligencia artificial local.
*   **Comentarios:** El docente publica el resumen generado mediante CU-27: Modificar material, una vez revisado su contenido.
...
*   **Precondición(es):**
    ...
    *   La unidad pertenece al programa vigente del curso.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente desea generar una presentación dentro de una unidad (ver CU-19: Buscar unidad).
    2.  El actor confirma la generación.
    3.  El sistema envía la bibliografía cargada de la unidad al modelo de inteligencia artificial local.
...
*   **Estabilidad:** Media — depende de la disponibilidad y el desempeño del modelo de inteligencia artificial local.
*   **Comentarios:** El docente publica la presentación generada mediante CU-27: Modificar material, una vez revisado su contenido, igual que en CU-69: Generar resumen de unidad.
...
*   **Estabilidad:** Alta
*   **Comentarios:** El guión se conserva para poder reutilizarlo si la clase se modifica y se regenera (ver CU-73: Modificar clase con Clon IA).
...
*   **Precondición(es):**
    ...
    *   La unidad existe, pertenece a un curso en el que el docente participa como titular o supervisor, y al programa vigente de dicho curso.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente desea generar una clase con Clon de IA dentro de una unidad (ver CU-19: Buscar unidad).
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el docente busca y selecciona la clase con Clon de IA a modificar (ver CU-71: Buscar clase con Clon IA).
...
### CU-74 Eliminar clase con Clon IA
*   **Objetivo(s) asociado(s):** OBJ-06: Generar contenido académico con inteligencia artificial.
*   **Requisito(s) de información asociado(s):** RI-06: Información sobre generación de contenido con inteligencia artificial.
*   **Módulo:** MOD-F-06: Módulo de Generación de Contenido con IA
*   **Actor(es):** Docente, Administrador
...
*   **Precondición(es):**
    ...
    *   La clase con Clon de IA existe, no se encuentra en baja, y pertenece a una unidad de un curso en el que el docente participa como titular o supervisor.
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la clase con Clon de IA a dar de baja (ver CU-71: Buscar clase con Clon IA).
    2.  El actor confirma la baja.
...
*   **Estabilidad:** Alta
*   **Comentarios:** Es la vía de autoregistro con correo y contraseña; el alta mediante Google OAuth se resuelve automáticamente en CU-84: Iniciar sesión (ver PA-1: Login con Google).
...
*   **Frecuencia:** Baja — se usa para altas manuales excepcionales; el alta habitual de un Alumno ocurre por autoregistro (CU-75) y la de un Docente por CU-82: Registrar docente.
*   **Estabilidad:** Alta
*   **Comentarios:** El Administrador puede crear cuentas de otros Administradores, pero no puede modificarlas.

### CU-78 Modificar usuario
*   **Objetivo(s) asociado(s):** OBJ-07: Gestionar usuarios, autenticación y notificaciones.
*   **Requisito(s) de información asociado(s):** RI-07: Información sobre usuarios y notificaciones.
*   **Módulo:** MOD-NF-01: Módulo de Usuarios y Notificaciones
*   **Actor(es):** Administrador
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la cuenta del alumno a modificar (ver CU-76: Buscar usuario).
...
*   **Estabilidad:** Alta
*   **Comentarios:** No incluye el cambio de contraseña, que se gestiona exclusivamente mediante CU-86: Recuperar contraseña. Los datos propios del Docente (información profesional) se gestionan mediante CU-83: Modificar docente, no con este CU.
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la cuenta del usuario a dar de baja (ver CU-76: Buscar usuario).
...
*   **Estabilidad:** Alta
*   **Comentarios:** Si el actor es Docente, también se muestran sus datos profesionales (biografía, años de experiencia, títulos, matrícula y estado de habilitación), aunque solo el Administrador puede modificarlos mediante CU-83: Modificar docente.
...
*   **Estabilidad:** Alta
*   **Comentarios:** No incluye el correo electrónico ni el cambio de contraseña, que se gestionan mediante CU-86: Recuperar contraseña. Si el actor es Docente, sus datos profesionales no se editan aquí; quedan a cargo del Administrador mediante CU-83: Modificar docente.
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona el docente a modificar (ver CU-76: Buscar usuario).
...
*   **Flujo de eventos:**
    1.  El caso de uso inicia cuando el actor busca y selecciona la sesión activa a cerrar (ver CU-87: Buscar sesión).
...
### CU-90 Generar informe de alumnos de un curso
...
### CU-91 Generar informe de ingresos de un curso
*   **Objetivo(s) asociado(s):** OBJ-09: Generar reportes y estadísticas de gestión.
*   **Requisito(s) de información asociado(s):** RI-09: Información sobre reportes y estadísticas.
*   **Módulo:** MOD-NF-03: Módulo de Reportes y Estadísticas
*   **Actor(es):** Administrador
...
*   **Frecuencia:** Baja — se usa puntualmente para ajustar el comportamiento del sistema.
*   **Estabilidad:** Alta
*   **Comentarios:** El esquema de clave-valor permite incorporar nuevos parámetros sin modificar el esquema de base de datos.